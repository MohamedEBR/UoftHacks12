...
def simulate_cat_vision_image(image_path, output_path):
    ...
    transformed_pixels = pixels @ CAT_TRANSFORMATION_MATRIX.T
...
def simulate_cat_vision_video(input_video_path, output_video_path):
    ...
    cat_vision_clip.write_videofile(output_video_path, codec="libx264", audio_codec="aac")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python catcolorblind.py <input_file_path> <output_file_path>")
        sys.exit(1)
    ...
    try:
        if input_file_path.lower().endswith(('.png', '.jpg', '.jpeg')):
            simulate_cat_vision_image(input_file_path, output_file_path)
        elif input_file_path.lower().endswith(('.mp4', '.avi', '.mkv', '.mov')):
            simulate_cat_vision_video(input_file_path, output_file_path)
        else:
