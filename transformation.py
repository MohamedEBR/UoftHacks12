...
def distance_blur(image, max_blur_radius, focal_point=None):
    """
    Apply a distance-based blur effect to an image using scipy.ndimage.gaussian_filter.
    
    Parameters:
        image (np.ndarray): Input image array (H x W x 3).
        max_blur_radius (int): Maximum radius (integer) for the blur.
        focal_point (tuple): (x, y) for the focal point. If None, uses image center.

    Returns:
        np.ndarray: Output image with distance-based blur.
    """
    # Image dimensions
...
# image = plt.imread("testingimg.jpeg", 10)
...
def dog_vision_array_transform(frame):
    ...
    # transformed = distance_blur(transformed, 1)
